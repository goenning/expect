...
func TestStringContainsString(t *testing.T) {
	...
	if !Expect("Hello World").ContainsString("World") {
		t.Error("Contains assertion failed")
	}

	if Expect("Hello World").ContainsString("John") {
		t.Error("Contains assertion failed")
	}
}
...
func TestTimeSimilar(t *testing.T) {
	...
	if !Expect(base).WithinTime(time1, 10*time.Second) {
		t.Error("WithinTime assertion failed")
	}

	if Expect(base).WithinTime(time1, time.Second) {
		t.Error("WithinTime assertion failed")
	}
}
